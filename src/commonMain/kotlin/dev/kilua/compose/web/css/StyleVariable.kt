...
import dev.kilua.browser.utils.titleCamelCaseToKebabCase
...
sealed class StyleVariable<T : StylePropertyValue, V>(
    ...
    class NumberValue<T : Number>(
        ...
        defaultFallback: T? = null,
        prefix: String? = null
    ) : StyleVariable<StylePropertyNumber, T>(
        name,
        defaultFallback?.let { StylePropertyValue(it) },
        prefix
    ) {
        override fun value(fallback: T?): T =
            variableValue(fallback?.let { StylePropertyValue(it) }).cast()
    }
    ...
    class StringValue(
        ...
        prefix: String? = null
    ) : StyleVariable<StylePropertyString, String>(
        name,
        defaultFallback?.let { StylePropertyValue(it) },
        prefix
    ) {
...
fun <T : StylePropertyValue> StyleScope.setVariable(
